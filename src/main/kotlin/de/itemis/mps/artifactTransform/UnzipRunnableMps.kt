...
abstract class UnzipRunnableMps : TransformAction<TransformParameters.None> {
    ...
    override fun transform(outputs: TransformOutputs) {
        ...
        // move the bin files
        val osSpecificBinFiles = File(
            outputDir, "bin/" + when (OperatingSystem.current()) {
        ...
        File(outputDir, "bin/fsnotifier").setExecutable(true)
        ...
        if (mpsVersion) {
            // move the JNA files
            val osSpecificJnaFiles: File? = File(
                outputDir, "lib/jna/" + when (System.getProperty("os.arch")) {
                    "aarch64" -> "aarch64"
                    "x86_64" -> "amd64"
                    "amd64" -> "amd64"
